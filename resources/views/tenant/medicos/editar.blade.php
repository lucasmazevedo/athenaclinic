<div class="modal-header">
    <h5 class="modal-title">Alterar Profissional ({{ 'Id.: ' . $model->id . ' - ' . $model->nome }})</h5>
            ...
            <div class="col-md-3">
                <div class="mb-3">
                    ...
                    <input type="text" id="dtnascimento" name="dtnascimento" class="form-control" placeholder=""
                        value="{{ dataPtbr($model->dtnascimento) }}">
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="mb-3">
                    ...
                    <select name="sexo" class="form-select">
                        <option value="1" @if ($model->sexo == 1) selected @endif>Masculino</option>
                        <option value="0" @if ($model->sexo == 0) selected @endif>Feminino</option>
                    </select>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    <select name="administrador" class="form-select">
                        @foreach (\App\Models\Tenant\Medico::listAdminStatus() as $ladmin)
                            <option value="{{ $ladmin['id'] }}" @if ($model->administrador['id'] == $ladmin['id']) selected @endif>
                                {{ $ladmin['text'] }}</option>
                        @endforeach
                    </select>
                </div>
...
<div class="modal-footer">
    <button type="button" class="btn btn-sm btn-danger" data-bs-dismiss="modal"><i class="fas fa-times me-2"></i>
        Cancelar</button>
    <a data-href="{{ route('profissionais.alterar', $model->id) }}" id="btnStoreModal" class="btn btn-sm btn-dark"><i
...
<script>
    ...
    Inputmask({
        "mask": (inputTelefone.value.length <= 10) ? '(99) 9999-9999' : '(99) 99999-9999'
    }).mask(inputTelefone);
